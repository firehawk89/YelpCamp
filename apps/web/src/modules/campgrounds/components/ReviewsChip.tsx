...
const ReviewsChip = ({ reviewsCount, campgroundSlug, className, ...props }: ReviewsChipProps) => (
  <Link
    href={routes.campground(`${campgroundSlug}#reviews`)}
    className={cn('bg-accent/20 hover:bg-accent/30 rounded-lg px-2 py-1 text-sm transition-all', className)}
    {...props}
  >
